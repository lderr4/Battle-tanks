        ...
        while dead == False and levels.currentLevel < len(levels.levelsArray):
            ...
            while running:
                ...
                if len(friendly_projectiles) != 0 or len(enemy_projectiles) != 0: # projectiles sprite group is not empty
                    ...
                    # collisions = pygame.sprite.groupcollide(friendly_projectiles, createMap.map_surfaces, True, False)
                    # for x in collisions:
                    #     if collisions[x][0].picture != "assets/blocks/yellow_block.png" and collisions[x][0].picture != "assets/blocks/purple_block.png":
                    #         createMap.map_surfaces.remove(collisions[x][0])
                    # collisions = pygame.sprite.groupcollide(enemy_projectiles, createMap.map_surfaces, True, False)
                    # for x in collisions:
                    #     if collisions[x][0].picture != "assets/blocks/yellow_block.png" and collisions[x][0].picture != "assets/blocks/purple_block.png":
                    #         createMap.map_surfaces.remove(collisions[x][0])

                    # collisions = pygame.sprite.groupcollide(friendly_projectiles, createMap.map_tanks, True, False)
                    # for x in collisions:
                    #     createMap.map_tanks.remove(collisions[x][0])
                    #     player.score += 50
                    #     if len(createMap.map_tanks.sprites())==0:
                    #         running = False

                    # collisions = pygame.sprite.groupcollide(enemy_projectiles, self_coll, True, False)
                    # if collisions:
